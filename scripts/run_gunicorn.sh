...
(cd gource_studio/; gunicorn -c ../config/gunicorn/prod.py)
sleep 1
...
if [ ! -f /var/run/gunicorn/prod.pid ]; then
    ...
    echo ""
    echo "  tail /var/log/gunicorn/prod.log"
    echo ""
    exit 2
fi

GUNICORN_PID=$(cat /var/run/gunicorn/prod.pid)
...
echo ""
echo "  tail -f /var/log/gunicorn/{access,error}.log"
echo ""
